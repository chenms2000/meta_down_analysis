# meta_down_analysis 使用说明

当前可用发布：`mvp_20260513T002254`

这个项目是一个研究原型，面向两类输入：上游分析得到的 GCST/trait 差异表，以及普通代谢物差异表。系统把输入连接到本地 GCST 注释、规范化代谢物实体、图谱投影、文献证据和可审计解释层，用于形成研究解释线索。该工具仍需进一步方法学验证和工程完善，不构成临床决策系统。
